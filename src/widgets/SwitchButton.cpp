...
SwitchButton::SwitchButton(QWidget* parent, Style style) : QWidget(parent), _value(false), _duration(100), _enabled(true) {
    ...
    _btn_move = new QPropertyAnimation(this);
    _back_move = new QPropertyAnimation(this);

    _btn_move->setTargetObject(_circle);
    _btn_move->setPropertyName("pos");
    _back_move->setTargetObject(_background);
    _back_move->setPropertyName("size");
...
void SwitchButton::paintEvent(QPaintEvent*) {
    auto const painter = new QPainter;
    painter->begin(this);
    painter->setRenderHint(QPainter::Antialiasing, true);

    QPen const pen(Qt::NoPen);
    painter->setPen(pen);
...
void SwitchButton::mousePressEvent(QMouseEvent*) {
    ...
    _btn_move->setDuration(_duration);
    _back_move->setDuration(_duration);

    constexpr int hback = 20;
    ...
    _btn_move->setStartValue(QPoint(xi, y));
    _btn_move->setEndValue(QPoint(xf, y));

    _back_move->setStartValue(initial_size);
    _back_move->setEndValue(final_size);
...
void SwitchButton::enable(bool flag) {
    _enabled = flag;
    _circle->enable(flag);
    _background->enable(flag);
    if (flag) _labelon->show();
    else {
        if (value()) {
            _labelon->show();
        }
...
void SwitchButton::setDuration(int time) { _duration = time; }
...
bool SwitchButton::value() const { return _value; }

void SwitchButton::_update() const {
    constexpr int hback = 20;
    QSize final_size(width() - 4, hback);
...
SwitchButton::SwitchBackground::SwitchBackground(QWidget* parent, QColor color, bool rect)
    : QWidget(parent), _rect(rect), _borderradius(12), _color(std::move(color)), _pencolor(QColor(170, 170, 170)) {
    setFixedHeight(20);
...
void SwitchButton::SwitchBackground::enable(bool flag) { _enabled = flag; }

SwitchButton::SwitchCircle::SwitchCircle(QWidget* parent, QColor color, bool rect)
    : QWidget(parent), _rect(rect), _borderradius(12), _color(std::move(color)), _pencolor(QColor(120, 120, 120)) {
    setFixedSize(20, 20);
...
void SwitchButton::SwitchCircle::enable(bool flag) { _enabled = flag; }